...
#         print ('yshape reshape: '+str(y.shape))
        ...
        #hs is faster than softmax, though it gives up a few percent
        #fasttext.supervised(train_filename, model_filename, loss='hs', silent=0)
#         fasttext.supervised(train_filename, model_filename, loss='hs', 
#                             silent=0, lr=.5, epoch=25, word_ngrams = 3, minn=2, maxn=6, min_count=3, bucket=2000000)
        fasttext.supervised(train_filename, model_filename, loss='hs', 
                            silent=0, lr=1, epoch=1000, word_ngrams = 3, minn=2, maxn=6, min_count=3, bucket=2000000)
...
#                 subset_test.append(line)
        ...
        atnum=10 #number of predictions to make for each test text
        print('predicting')
